...
class DecoderBlock(nn.Module):
    def __init__(self, low_level_channels, encoder_channels, out_channels=256):
...
class ResNetEncoder(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.layer3 = self._make_layer(128, 256, 6, stride=1, dilation=2)
        self.layer4 = self._make_layer(256, 512, 3, stride=1, dilation=4)
    ...
    def _make_layer(self, in_channels, out_channels, blocks, stride=1, dilation=1):
        ...
        layers = []
        layers.append(ResNetBlock(in_channels, out_channels, stride, dilation, downsample))
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        low_level_feat = x  # layer1 features for decoder
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, num_classes=19, in_channels=3):
        ...
        # Encoder
        self.backbone = ResNetEncoder(in_channels=in_channels)
        
        # ASPP module
        self.aspp = ASPP(512, [12, 24, 36])
        
        # Decoder
        self.decoder = DecoderBlock(64, 256)
    ...
    def forward(self, x):
        ...
        x = self.classifier(decoder_features)
